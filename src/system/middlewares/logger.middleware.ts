...
class LoggerRequestMiddleware implements NestMiddleware {
  ...
  use(
    ...
    response.on('finish', () => {
      ...
      // const message = `${method} ${originalUrl} ${statusCode} ${statusMessage} ${processingTime}ms ${request.headers['useremail']} ${request.headers['tenantid']}`;
